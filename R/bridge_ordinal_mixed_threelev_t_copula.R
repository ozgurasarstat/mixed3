bridge_ordinal_mixed_threelev_t_copula = "
...
for(i in 1:nsubj){
z_vec[ind_s[i, 1]:ind_s[i, 2]] = sqrt(W[i]) * chol_covmat_exp(time[ind_s[i, 1]:ind_s[i, 2]], nrepeat_s[i], kappa) * zstar[ind_s[i, 1]:ind_s[i, 2]];
}

for(i in 1:ntot){
v_vec[i] = inv_cdf_bridge(student_t_cdf(z_vec[i], nu, 0, 1), phi_v);
}
