...
fit_mixed3 <- function(formula,
  ...
  }else if(reff$U != "none" & reff$V != "none"){
    data <- data[order(data[, c_id], data[, s_id]), ]
  }
  ...
  ## response, covariate matrices, total number of observations
  ...
  if(model == "normal"){
    if(reff$U == "serial" & reff$V == "serial"){
      ...
      mod <- stan_model(model_code = mixed_normal_ordinal_intercept_intercept, auto_write = TRUE)
    }
    ...
    if(reff$U == "serial" & reff$V == "serial"){
      ...
      mod <- stan_model(model_code = mixed_bridge_ordinal_intercept_intercept, auto_write = TRUE)
    }else if(reff$U == "none" & reff$V == "serial"){
      mod <- stan_model(model_code = mixed_bridge_ordinal_none_serial, auto_write = TRUE)
    }else if(reff$U == "none" & reff$V == "intercept"){
      mod <- stan_model(model_code = mixed_bridge_ordinal_none_intercept, auto_write = TRUE)
    }
  }else if(model == "fixed"){
    mod <- stan_model(model_code = ordinal_fixed, auto_write = TRUE)
  }
